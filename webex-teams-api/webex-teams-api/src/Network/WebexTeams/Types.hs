...
$(deriveJSON defaultOptions { constructorTagModifier = dropAndLow 20 } ''WebhookNotifyOwnedBy)
...
data WebhookNotifyData = WebhookNotifyDataMsg Message
                       | WebhookNotifyDataRoom WebhookNotifyRoom
                       | WebhookNotifyDataMembership WebhookNotifyMembership
                       deriving (Eq, Generic, Show)

-- $(deriveJSON defaultOptions ''WebhookNotifyData)
...
data WebhookNotify = WebhookNotify
    ...
    , webhookNotifyTargetUrl :: WebhookUrl
    , webhookNotifyResource  :: WebhookResource
    , webhookNotifyEvent     :: WebhookEvent
    , webhookNotifyFilter    :: Maybe WebhookFilter
    , webhookNotifyOrgId     :: OrganizationId
    ...
    , webhookNotifyCreated   :: Timestamp
    , webhookNotifyActorId   :: PersonId
    , webhookNotifyData      :: WebhookNotifyData
    } deriving (Eq, Show)

instance FromJSON WebhookNotify where
    parseJSON v@(Object o) = do
        id <- o .: "id"
        name <- o .: "name"
        targetUrl <- o .: "targetUrl"
        resource <- o .: "resource" :: Parser WebhookResource
        event <- o .: "event"
        maybeFilter <- o .:? "filter"
        orgId <- o .: "orgId"
        ...
        let d' = case resource of
                WebhookResourceMessages    -> WebhookNotifyDataMsg d
                --WebhookResourceRooms       -> WebhookNotifyDataRoom d
                --WebhookResourceMemberships -> WebhookNotifyDataMembership d

        pure $ WebhookNotify id name targetUrl resource event maybeFilter orgId createdBy appId ownedBy status created actorId d'
